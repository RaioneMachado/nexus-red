...
interface ContentItem {
  ...
  genre: string;
  duration?: string;
  aspectRatio?: string; // Novo campo opcional para definir o aspecto da imagem
}
      ...
      <section id={id} className="py-12 md:py-20">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
            {/* Featured Item */}
            <div className="relative w-full h-[40vh] sm:h-[50vh] lg:h-auto lg:row-span-2 rounded-xl md:rounded-2xl overflow-hidden group">
              <div className="relative w-full h-full">
                <img
                  src={featured.image}
                  alt={featured.title}
                  className="w-full h-full object-contain lg:object-cover transition-all duration-700 group-hover:scale-105"
                />
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 gap-4">
              {rest.map((item) => (
                ...
                >
                  <div className="relative pb-[150%] sm:pb-[150%] rounded-lg overflow-hidden group cursor-pointer transform transition-all duration-500 hover:scale-105 hover:z-10">
                    <img
                      src={item.image}
                      alt={item.title}
                      className="absolute inset-0 w-full h-full object-contain lg:object-cover transition-transform duration-500 group-hover:scale-110"
                    />
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6">
          {items.map((item) => (
            <div
              ...
              className="relative w-full"
            >
              <div className="relative pb-[150%] rounded-lg overflow-hidden group cursor-pointer transform transition-all duration-500 hover:scale-105 hover:z-10">
                <img
                  src={item.image}
                  alt={item.title}
                  className="absolute inset-0 w-full h-full object-contain lg:object-cover transition-transform duration-500 group-hover:scale-110"
                ...
                {/* Static Overlay - Apenas para imagens horizontais */}
                ...
                <div className="absolute inset-0 flex flex-col justify-center items-center opacity-0 group-hover:opacity-100 transition-all duration-500 bg-black/80">
                  <div className="text-center mb-4 px-4">
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-3 text-sm text-gray-300 font-sans mb-4">
